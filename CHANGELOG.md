...
选区一览里多个未命中词走 `fallback_many` 并行走云端（每词仍单独 HTTP），列表先全部标查询中，谁先返回谁先填；事件与入库都带本次 seq，过期丢弃（在飞 HTTP 不中止）。新 grab 作废在飞写入，`FALLBACK_SEQ` 只单调前进，`fallback*` 在 spawn 前捕获 epoch，持锁后再确认 live。非法词或单任务失败只让该行失败，不中断整批。upsert 失败不把未入库结果标成 pending。云端耗时写入 query_log（`cloud*`），不计入本地 p95。云端提示词改为单术语注释（非整句翻译）：无上下文按 AI 编程默认域；标识符/无通行译名 keep 或 note，禁止编造译法。
