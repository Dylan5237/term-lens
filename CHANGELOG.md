...
选区一览里多个未命中词走 `fallback_many` 并行走云端（每词仍单独 HTTP），列表先全部标查询中，谁先返回谁先填；事件带本次 seq，过期丢弃。非法词只让该行失败，不中断整批。云端提示词改为单术语注释（非整句翻译）：无上下文按 AI 编程默认域；标识符/无通行译名 keep 或 note，禁止编造译法。
