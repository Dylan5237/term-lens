...
选区一览里多个未命中词走 `fallback_many` 并行走云端（每词仍单独 HTTP），列表先全部标查询中，谁先返回谁先填；事件与入库都带本次 seq，过期丢弃。新 grab 取消在飞 HTTP 并作废写入，同时最多 8 路云端请求。`FALLBACK_SEQ` 只单调前进，`fallback*` 在 spawn 前捕获 epoch，持锁后再确认 live。非法词或单任务失败只让该行失败，不中断整批。upsert 失败不把未入库结果标成 pending。完成的云端请求写入 query_log（`cloud*`），取消不记，不计入本地 p95。云端提示词改为单术语注释（非整句翻译）：无上下文按 AI 编程默认域；标识符/无通行译名 keep 或 note，禁止编造译法。

## 0.1.6 — 2026-09-16

同译同域的官方层与个人层合并为一张卡，并列层标签，不再标「多义」。句子内连续英文收成短语，不再隔着中文做邻接排列组合。悬浮窗用已查术语回填选区做整句对照（整句不上云）。README 按现役能力重写。

## 0.1.5 — 2026-09-11

选区抽出 2 个及以上术语时改为未知优先列表：点开一行在嵌套块里看释义并裁决（「当前语境」置顶），单术语仍走原单卡。明文 http 除 loopback 外允许 RFC1918 字面量 IP，便于接内网 OpenAI 兼容网关；默认仍不上云。

## 0.1.4 — 2026-09-10

悬浮窗不再裁切释义：窗口随内容增高（最高 640px，且不超过工作区 2/3）。Win32 工作区贴边碰撞，下方不够则翻到指针上方。短释义不出现滚动条；仅超长释义在释义块内滚动。
