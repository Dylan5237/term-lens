...
- 系统提示词：内置默认 `src-tauri/src/fallback_prompt.md`；运行时读写 `%APPDATA%\term-lens\fallback_prompt.md`（每次调用时读取，文件即配置=热重载）。**仓库里没有 `prompts/fallback.md`。** 提示词按划词注释器写：只注释单个未命中术语；无上下文时用 AI 编程默认域；禁止整句翻译与幻觉译名。
...
- 术语提取：snake/kebab/camel 整段当一词；选区本身像术语（≤4 词段、无句号/换行/列表符）则整段优先；句子与列表只抽内部标识符/短语。连续英文仅空白相连时收成一条（≤4 词），中文或标点切断，禁止隔字 bigram 排列组合。一览最多 8 词。
- 多义：分不出域则平铺全部释义，不点选、不猜、不上云。同译同域跨层合并为一张卡，层标签并列。
- 选区一览：≥2 个术语时只留计数头；未知优先列表，点开一行在嵌套块里看释义并裁决（「当前语境」置顶）；1 个仍单卡。卡片顶部用已查译法做整句对照，禁止把整句送上云。云端只传未命中单词、`fallback_many` 并行 HTTP（每词单独请求，事件与 upsert 都带本次 seq，过期丢弃），禁止批量采纳 pending。新 grab 作废在飞写入；`FALLBACK_SEQ` 只单调前进；`fallback*` 在 spawn 前捕获 epoch。非法词或单任务失败只让该行失败，不中断整批。upsert 失败不展示未入库结果。云端耗时写入 query_log（`cloud*`），不计入本地 p95。
...
- 版本号：Cargo.toml、tauri.conf.json、README 安装说明统一 **0.1.7**。
