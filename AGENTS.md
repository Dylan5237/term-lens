...
- 选区一览：≥2 个术语时只留计数头；未知优先列表，点开一行在嵌套块里看释义并裁决（「当前语境」置顶）；1 个仍单卡。卡片顶部用已查译法做整句对照，禁止把整句送上云。云端只传未命中单词、`fallback_many` 并行 HTTP（每词单独请求，事件带本次 seq，过期丢弃），禁止批量采纳 pending。非法词只让该行失败，不中断整批。
