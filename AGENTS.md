...
- 选区一览：≥2 个术语时只留计数头；未知优先列表，点开一行在嵌套块里看释义并裁决（「当前语境」置顶）；1 个仍单卡。卡片顶部用已查译法做整句对照，禁止把整句送上云。云端只传未命中单词、`fallback_many` 并行 HTTP（每词单独请求，事件与 upsert 都带本次 seq，过期丢弃），禁止批量采纳 pending。新 grab 取消在飞 HTTP 并作废写入；同时最多 8 路云端请求。`FALLBACK_SEQ` 只单调前进；`fallback*` 在 spawn 前捕获 epoch。非法词或单任务失败只让该行失败，不中断整批。upsert 失败不展示未入库结果。完成的云端请求写入 query_log（`cloud*`），取消不记，不计入本地 p95。
