...
> **产品冻结**：不支持终端取词。取词优先 UI Automation / 原生 Edit（多数窗口不碰剪贴板）。Cursor 等读不到时才短暂复制并立刻还原。读不到选区才失败。默认无云端；没配好 loopback / https 白名单就不发外网。密钥走 Windows 凭据管理器，不要写进 `config.toml`。
