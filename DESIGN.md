...
> Windows 托盘划词注释器 · v0.1.6
> 日期：2026-09-16 · 作者：森几 × 小墨
> v0.1.6 变更：短语提取不再隔字排列组合；同译跨层并排标签；整句对照不上云。产品仍冻结为划词注释器（非终端透镜），默认无云端
...
**产品冻结（自 0.1.2；当前发版 0.1.6）**：这是 **Windows 托盘划词注释器**，不是终端透镜。不支持终端取词。取词优先 UI Automation / 原生 Edit；Electron 等读不到时才短暂 Ctrl+C，立刻还原剪贴板（非文本剪贴板不回退）。读不到选区才失败；同一词再划一次仍查询。默认无云端；没配好 loopback/白名单就不发外网。
...
- **选区一览**：抽出 2 个及以上术语时，卡片只留计数头（n 个未知 / n 个已知），不复用单卡的层/域标题。未知（未命中/待确认/多义未裁决）在上、已知一行收起；点开一行在嵌套块里看释义并裁决（按钮跟该行走），不同时展开两行。多义时「当前语境」置顶，其余释义列在同一嵌套块内，不与术语行平级。展开/收起只改高度并贴边，不按鼠标重锚窗口。1 个术语仍走单卡。最多展示 8 个，超出只报「还有 n 个未列」。禁止批量采纳云端 pending。卡片顶部用已命中译法回填选区做**整句对照**（不把整句送上云）
- **上下文开关**：删除。云端永远只传术语单词
