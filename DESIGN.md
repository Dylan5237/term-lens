...
**产品冻结（自 0.1.2；当前发版 0.1.3）**：这是 **Windows 托盘划词注释器**，不是终端透镜。不支持终端取词。取词优先 UI Automation / 原生 Edit；Electron 等读不到时才短暂 Ctrl+C，立刻还原剪贴板（非文本剪贴板不回退）。读不到选区才失败；同一词再划一次仍查询。默认无云端；没配好 loopback/白名单就不发外网。
...
- **0.1.3（当前发版）**：UIA 优先取词；snake/kebab/camel 与短选区整段查询；多义平铺
