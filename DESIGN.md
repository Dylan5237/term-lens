...
> Windows 托盘划词注释器 · v0.1.7
> 日期：2026-09-18 · 作者：森几 × 小墨
> v0.1.7 变更：未命中词并行云端兜底；提示词改为单术语注释。产品仍冻结为划词注释器（非终端透镜），默认无云端
...
**产品冻结（自 0.1.2；当前发版 0.1.7）**：这是 **Windows 托盘划词注释器**，不是终端透镜。不支持终端取词。取词优先 UI Automation / 原生 Edit；Electron 等读不到时才短暂 Ctrl+C，立刻还原剪贴板（非文本剪贴板不回退）。读不到选区才失败；同一词再划一次仍查询。默认无云端；没配好 loopback/白名单就不发外网。
...
**非目标（冻结范围内明确不做）**
- 整页/整篇文档翻译；整句送上云机翻（本地用已查术语回填对照：**已实施**）
- 云端整段流式跟翻
...
| 云端兜底 | 默认关闭。可选：loopback / RFC1918 内网 OpenAI 兼容，或 **https** 公网 | 未配置 / URL 不在允许名单 → 不发请求 |
...
│        有选区（含同一词再划）→ cap 8–16KB → 预提取英文片段（CJK 紧贴可抽；snake/kebab/camel 整段一词；短选区≤4 词段整段优先；**句子内仅空白相连的英文收成一条短语（≤4 词），中文/标点切断，禁止隔字排列组合**；停用词切开短语；最多 8 词，超出只计数）
│        ├─ 一级：本地术语表（归一化 + 域裁决）    命中 <200ms
│        │         多候选且分差为 0 → hit=None，并列展示，不上云
│        └─ 二级：云端兜底（仅传未命中的术语单词，H1；未配置则失败且不发请求）
...
- **多候选裁决**（P2）：命中多域候选时按上下文关键词打分选域；`best==0` 且**不同译法/域**则 **不猜**，悬浮窗平铺全部释义（不点选）；有命中时仍平铺并标「当前语境」。同译同域的跨层行（如官方+个人）算一条释义，一张卡上并列层标签，不标「多义」
- **悬浮窗**：释义全文展示；窗口随内容增高（上限 640px，且不超过工作区高度的 2/3）。用 Win32 工作区做四边碰撞，下方不够则翻到指针上方。卡片本身不出现滚动条；仅超长释义或术语列表内滚动
- **选区一览**：抽出 2 个及以上术语时，卡片只留计数头（n 个未知 / n 个已知），不复用单卡的层/域标题。未知（未命中/待确认/多义未裁决）在上、已知一行收起；点开一行在嵌套块里看释义并裁决（按钮跟该行走），不同时展开两行。多义时「当前语境」置顶，其余释义列在同一嵌套块内，不与术语行平级。展开/收起只改高度并贴边，不按鼠标重锚窗口。1 个术语仍走单卡。最多展示 8 个，超出只报「还有 n 个未列」。云端对未命中词**并行**请求（每词单独传单词，禁止拼成一条 prompt、禁止批量采纳 pending）。卡片顶部用已命中译法回填选区做**整句对照**（不把整句送上云）
- **上下文开关**：删除。云端永远只传术语单词
...
- **M0 / 0.1.2**：产品冻结——Windows 托盘划词闭环；默认零外网
- **0.1.3**：UIA 优先取词；snake/kebab/camel 与短选区整段查询；多义平铺
- **0.1.4**：悬浮窗全文展示释义；最高 640px / 工作区 2/3；边缘碰撞
- **0.1.5**：选区术语一览；RFC1918 明文 http
- **0.1.6**：短语提取、同译层合并、整句对照
- **0.1.7（当前发版）**：未命中词并行兜底；云端提示词改为单术语注释
...
- 数据出境：H1 默认无云端；开启后仅术语单词；loopback / RFC1918 走 no_proxy，外网走系统代理
