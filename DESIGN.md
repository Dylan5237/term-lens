...
> Windows 托盘划词注释器 · v0.1.5
> 日期：2026-09-11 · 作者：森几 × 小墨
> v0.1.5 变更：多术语选区一览；明文 http 允许 RFC1918 内网网关。产品仍冻结为划词注释器（非终端透镜），默认无云端
...
**产品冻结（自 0.1.2；当前发版 0.1.5）**：这是 **Windows 托盘划词注释器**，不是终端透镜。不支持终端取词。取词优先 UI Automation / 原生 Edit；Electron 等读不到时才短暂 Ctrl+C，立刻还原剪贴板（非文本剪贴板不回退）。读不到选区才失败；同一词再划一次仍查询。默认无云端；没配好 loopback/白名单就不发外网。
...
│        有选区（含同一词再划）→ cap 8–16KB → 预提取英文片段（CJK 紧贴可抽；snake/kebab/camel 整段一词；短选区≤4 词段整段优先；停用词不作为第一优先；最多 8 词，超出只计数）
...
- **多候选裁决**（P2）：命中多域候选时按上下文关键词打分选域；`best==0` 且多候选则 **不猜**，悬浮窗平铺全部释义（不点选）；有命中时仍平铺并标「当前语境」
- **悬浮窗**：释义全文展示；窗口随内容增高（上限 640px，且不超过工作区高度的 2/3）。用 Win32 工作区做四边碰撞，下方不够则翻到指针上方。卡片本身不出现滚动条；仅超长释义或术语列表内滚动
- **选区一览**：抽出 2 个及以上术语时，卡片只留计数头（n 个未知 / n 个已知），不复用单卡的层/域标题。未知（未命中/待确认/多义未裁决）在上、已知一行收起；点开一行在嵌套块里看释义并裁决（按钮跟该行走），不同时展开两行。多义时「当前语境」置顶，其余释义列在同一嵌套块内，不与术语行平级。展开/收起只改高度并贴边，不按鼠标重锚窗口。1 个术语仍走单卡。最多展示 8 个，超出只报「还有 n 个未列」。禁止批量采纳云端 pending
- **上下文开关**：删除。云端永远只传术语单词
...
- **0.1.4（当前发版）**：悬浮窗全文展示释义；最高 640px / 工作区 2/3；边缘碰撞
