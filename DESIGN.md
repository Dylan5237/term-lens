...
> Windows 托盘划词注释器 · v0.1.4
> 日期：2026-09-10 · 作者：森几 × 小墨
> v0.1.4 变更：悬浮窗全文展示释义并随内容增高；产品仍冻结为划词注释器（非终端透镜），默认无云端
...
**产品冻结（自 0.1.2；当前发版 0.1.4）**：这是 **Windows 托盘划词注释器**，不是终端透镜。不支持终端取词。取词优先 UI Automation / 原生 Edit；Electron 等读不到时才短暂 Ctrl+C，立刻还原剪贴板（非文本剪贴板不回退）。读不到选区才失败；同一词再划一次仍查询。默认无云端；没配好 loopback/白名单就不发外网。
...
- **悬浮窗**：释义全文展示；窗口随内容增高（上限 640px，且不超过工作区高度的 2/3）。用 Win32 工作区做四边碰撞，下方不够则翻到指针上方。卡片本身不出现滚动条；仅超长释义在 note / 多义列表内滚动
- **上下文开关**：删除。云端永远只传术语单词
